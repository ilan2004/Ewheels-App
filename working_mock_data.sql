-- Working Floor Manager Mock Data Script
-- Uses existing user IDs to satisfy foreign key constraints

-- =================================================================
-- 1. FIND EXISTING USER AND CLEAN UP
-- =================================================================

-- First, let's find an existing user ID from your database
DO $$
DECLARE 
    existing_user_id uuid;
    location_uuid uuid;
BEGIN
    -- Try to find an existing user from service_tickets
    SELECT created_by INTO existing_user_id FROM service_tickets WHERE created_by IS NOT NULL LIMIT 1;
    
    -- If no user found in service_tickets, try to find from profiles
    ...
    INSERT INTO temp_user VALUES (COALESCE(existing_user_id, gen_random_uuid()));
    INSERT INTO temp_location VALUES (COALESCE(location_uuid, gen_random_uuid()));
    
    RAISE NOTICE 'Using user ID: %, location ID: %', existing_user_id, location_uuid;
END $$;
...
-- =================================================================
-- 3. CREATE TEST USERS FOR FLOOR MANAGER
-- =================================================================
...
-- =================================================================
-- 4. CREATE UNASSIGNED SERVICE TICKETS
-- =================================================================
...
SELECT 
  ...
  priority,
  (SELECT id FROM temp_location),
  (SELECT id FROM temp_user),  -- Use existing user
  (SELECT id FROM temp_user),  -- Use existing user
  created_time,
  created_time
FROM (VALUES 
...
-- =================================================================
-- 5. CREATE ASSIGNED SERVICE TICKETS 
-- =================================================================
...
SELECT 
  ...
  priority,
  (SELECT id FROM temp_location),
  (SELECT id FROM temp_user),  -- created_by
  '11111111-1111-1111-1111-111111111111'::uuid, -- updated_by (floor manager)
  technician_id,  -- assigned_to
  '11111111-1111-1111-1111-111111111111'::uuid, -- assigned_by (floor manager)
  assigned_time,
  created_time,
  assigned_time
FROM (VALUES 
  -- John's tickets (6 tickets - near capacity)
  ...
  -- Sarah's tickets (2 tickets - oldest assignment)
